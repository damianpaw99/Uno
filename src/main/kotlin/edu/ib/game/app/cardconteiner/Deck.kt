...
class Deck : CardContainer() {
    ...
    companion object {
        fun generateFullDeck(x: Double, y: Double):Deck {
            ...
            for (i in 1 until 9) {
                for (j in 0 until 3) {
                    deck.addCard(NormalCard(CardColor.getColor(j), i, "${CardColor.getColor(j)} ${i}")) //karty zwykłe podwójne
                    deck.addCard(NormalCard(CardColor.getColor(j), i, "${CardColor.getColor(j)} ${i}"))
                }
            }
            for (i in 0 until 3) {
                deck.addCard(NormalCard(CardColor.getColor(i), 0, "${CardColor.getColor(i)} 0")) //karty 0 pojedyncze
            }
            //karty specjalne
